...
class ImapEmailService {
	...
	public function listUnseen($username, $password, $max) {
		return $this->listFromImap($username, $password, $max, 'UNSEEN');
	}
	...
	public function list($username, $password, $max, $criteria = 'ALL') {
		...
		foreach ($emails as $key => $email_number) {
			...
			$remetente = $this->decode_imap_from($email->from);
			$assunto = $this->decode_imap_subject($email->subject);
			$dataHoraEmail = $this->decode_imap_date($email->date);

			date_default_timezone_set("America/Sao_Paulo");
			$dataHoraImportacao = date("Y-m-d H:i:s");

			$conteudo = $this->decode_imap_body($mailbox, $email_number);
	...
	private function decode_imap_subject($subject) {
		return $this->imap_utf8_fix($subject);
	}

	private function decode_imap_from($from) {
	...
	private function decode_imap_date($date) {
	...
	private function decode_imap_body($imap, $uid) {
